...
export class Character extends GameEntity {
  ...
  constructor({ posX, posY }: CharacterProps, tiles: Tiles) {
    super({
      posX,
      posY,
      width: Tile.size - 4,
  ...
  moveCharacterAutomatically() {
    ...
    this.levelUpMessage = "Congratulations Few More TO GO!";
  }
  ...
  handleCollision() {
    ...
    for (let tile of this.tiles.solid) {
      this.handleSolidTileCollision(tile);
    }

    for (let i = 0; i < this.tiles.edible.length; i++) {
      const tile = this.tiles.edible[i];
    ...
    for (let tile of this.tiles.harming) {
  ...
  private handleEdibleTileCollision(tile: EdibleTile, index: number) {
    ...
    if (tile.type === "door") {
      ...
      this.tiles.edible.splice(index, 1);
    }
